...
--3.Afficher le nom, date d'embauche, le numéro du supérieur, le numéro de departement et le salaire de tous les employés.
...
--8.Afficher le nom et le prénom des employés dont le nom est alphabéthiquement antérieur au prénom.
...
--10.Afficher le nom, le titre et le salaire des employés dont le titre est "représentant" ou dont le titre est "président".ADD

SELECT nom, titre, salaire FROM employe WHERE titre = "représentant" || titre = "président";
...
SELECT nom, titre, nodep, salaire FROM employe WHERE nodep= 34 AND (titre="représentant" || titre = "secrétaire");
...
SELECT nom, titre, nodep, salaire FROM employe WHERE (titre = "représentant" || titre = "secrétaire") && nodep=34;
...
SELECT nom, (salaire * tauxcom) as com, salaire,tauxcom FROM employe WHERE tauxcom IS NOT NULL ;
...
SELECT nom, (salaire * tauxcom) as com, salaire, tauxcom FROM employe WHERE tauxcom IS NOT NULL order by tauxcom asc;

--28. Afficher le nom et le prénom (concaténés) des employés. Renommer les colonnes.

SELECT noemp, CONCAT(nom,' ', prenom) AS "identité" FROM employe;

--OU

SELECT CONCAT(nom,' ', prenom) AS "identité" FROM employe;
...
SELECT locate("r", nom) FROM employe WHERE locate("r", nom)>1;
